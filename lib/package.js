...
module.exports = function (opts, cb) {
    ...
    gulp.task("clean-app-package", function () {
        return del(path.join(cwd, "/deploy/") + "*.zip");
    });
    ...
    if (packageFile.packagedDependencies) {
        gulp.task("version-increment", [
            "clean-app-package",
            "package-modules"
        ], versionIncrement);
    } else {
    ...
    gulp.on("task_stop", function (msg) {
        if (msg.task == "build-prod") {
            ...
            gulp.util.log(gulp.colors.red("The eagle is in the nest."));
        }
    });
    ...
    if (packageBuildTarget) {
        gulp.task("build-and-package", function (cb) {
            require("./build")(packageBuildTarget, opts, function () {
                runSequence("build-steps-" + packageBuildTarget, function () {
                    loadPackage(cwd, function (config) {
                        generatePackageInclude(config);
                        gulp.util.log("Starting a build and package for target '" + packageBuildTarget + "' with " + packageInclude + " dependencies");
        ...
        gulp.util.log("No package build target, so just packaging...", packageInclude);
        if (cb) {
            cb(undefined, runCmd);
        }
    }

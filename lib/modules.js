...
module.exports = function(cmd, opts, cb) {
    ...
    gulp.task("build-modules", ["explode-modules"], function(cb) {
        if (explodeModules.length > 0) {
            exec("__dirname + '/../node_modules/.bin/npm rebuild", {
                maxBuffer: 10000 * 1024
            }, function(err, stdout, stderr) {
                console.log(stdout);
                console.error(stderr);
    ...
    gulp.task('install-private-modules', ['install-public-modules'], function(cb) {
        ...
        if (deps.length > 0) {
            ...
            child = exec(__dirname + '/../node_modules/.bin/npm install ' + installDeps.join(" ") + " --registry " + uri, function(err) {
                cb();
            });
    ...
    gulp.task('install-public-modules', function(cb) {
        ...
        child = exec(__dirname + '/../node_modules/.bin/npm install ' + publicInstallDeps.join(" "), function(err) {
            cb();
        });
        child.stdout.pipe(process.stdout);
        child.stderr.pipe(process.stderr);

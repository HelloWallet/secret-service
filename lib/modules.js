...
module.exports = function(cmd, opts, cb) {
    ...
    gulp.task("check-modules", function(cb) {
        ...
        glob.glob('modules/' + packages + '.zip', function(err, modules) {
            ...
            var q = async.queue(function (file, callback) {
                ...
                ex.extract({path: dir, filter: function(file) {
                    var package = file &&
            ...
            q.push(_.map(modules, function(m) { return { name: m, base: "node_modules"} }));
        });
    });
    ...
    gulp.task('clean-packaged-modules', function() {
        ...
        return gulp.src("modules/" + packages + ".zip", {cwd: opts.path, read: false})
            .pipe(clean());
    });

    gulp.task("packdeps-modules", function(cb) {
        exec(__dirname + "/../node_modules/.bin/pac -s npm", {maxBuffer: 10000*1024}, function(err, stdout, stderr) {
            console.log(stdout);
            console.error(stderr);
            exec(__dirname + "/../node_modules/.bin/pac -s bower", {maxBuffer: 10000*1024}, function(err, stdout, stderr) {
                console.log(stdout);
                console.error(stderr);
                cb(err);
            });
        ...
        glob.glob(packages, function(err, modules) {
            ...
            var streams = modules.map(function(dir) {
                ...
                if (skip) {
                    ...
                    return gulp.src(glob, {cwd: path.join(cwd, dir)})

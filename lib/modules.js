...
module.exports = function(cmd, opts, cb) {
        ...
        npm.load(function(er, npm) {
            ...
            npm.registry.adduser(uri, params, function(er, doc) {
                ...
                //npm.config.save("user", function(err) {
                    ...
                    // if (opts.package) {
                    //     npm.commands.publish([opts.package], cb);
                    // } else {
    ...
    gulp.task('install-private-modules', ['install-public-modules'], function(cb) {
        ...
        if (deps.length > 0) {
            ...
            child = exec('npm install ' + installDeps.join(" ") + " --registry " + uri, function(err) {
                cb();
            });
    ...
    gulp.task('install-public-modules', function(cb) {
        ...
        child = exec('npm install ' + publicInstallDeps.join(" "), function(err) {
            cb();
        });
        child.stdout.pipe(process.stdout);
        child.stderr.pipe(process.stderr);
